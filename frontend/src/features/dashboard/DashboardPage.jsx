...
import { dashboardApi, warehouseApi } from '@/api';
...
export default function DashboardPage() {
  ...
  const BAR_COLOR = palette.primary;

  // Flat KPI card: white surface, ink number, a thin teal accent strip on top.
  // The receivables card flips to a danger accent only when there is overdue debt.
  const kpiCardStyle = (accent = palette.primary) => ({
  ...
  return (
    <div style={{ padding: 24, background: palette.canvas, minHeight: '100vh' }}>
      <Tabs
        activeKey={activeTab}
    ...
    return (
      <>
        ...
        <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
          <Col xs={24} sm={12} md={6}>
            <Card bordered={false} style={kpiCardStyle()}>
              <Statistic
                ...
                prefix={<DollarOutlined style={{ color: palette.primary }} />}
                suffix="₫"
                valueStyle={{ color: palette.ink, fontSize: 22 }}
                formatter={(v) => fmt(v)}
              />
              {summary.revenueGrowthPercent != null && (
                <div style={{ marginTop: 8, fontSize: 13, color: growthColor }}>
          ...
          <Col xs={24} sm={12} md={6}>
            <Card bordered={false} style={kpiCardStyle()}>
              <Statistic
                ...
                prefix={<InboxOutlined style={{ color: palette.primary }} />}
                suffix="₫"
                valueStyle={{ color: palette.ink, fontSize: 22 }}
                formatter={(v) => fmt(v)}
              />
          ...
          <Col xs={24} sm={12} md={6}>
            {(() => {
              const overdue = summary.totalOverdueReceivables > 0;
              const accent = overdue ? palette.danger : palette.primary;
              return (
            <Card bordered={false} style={kpiCardStyle(accent)}>
              <Statistic
                title="Công nợ chưa thu"
                value={summary.totalReceivables}
                prefix={<FileTextOutlined style={{ color: accent }} />}
                suffix="₫"
                valueStyle={{ color: palette.ink, fontSize: 22 }}
                formatter={(v) => fmt(v)}
              />
              <div style={{ marginTop: 8, fontSize: 13, color: overdue ? palette.danger : palette.muted }}>
        ...
        {topProducts.map((product, idx) => (
          <div key={idx} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <Text strong style={{ fontSize: 13 }}>
                <Badge count={idx + 1} size="small"
                  style={{
                    backgroundColor: idx < 3 ? palette.primary : '#d9d9d9',
                    marginRight: 8
                  }}
                />
                {product.productName}
            ...
            <Progress
              ...
              strokeColor={idx < 3 ? palette.primary : '#8c8c8c'}
              size="small"
            />
          </div>
        ))}

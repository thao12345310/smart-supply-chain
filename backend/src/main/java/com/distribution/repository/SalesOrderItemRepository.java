...
public interface SalesOrderItemRepository extends JpaRepository<SalesOrderItem, Long> {
    ...
    List<SalesOrderItem> findPendingDeliveryBySalesOrderId(@Param("salesOrderId") Long salesOrderId);
    
    @Query("SELECT SUM(soi.quantity) FROM SalesOrderItem soi WHERE soi.salesOrder.id = :salesOrderId")
    Integer getTotalQuantityBySalesOrderId(@Param("salesOrderId") Long salesOrderId);
    
    @Query("SELECT SUM(soi.deliveredQuantity) FROM SalesOrderItem soi WHERE soi.salesOrder.id = :salesOrderId")
    Integer getTotalDeliveredQuantityBySalesOrderId(@Param("salesOrderId") Long salesOrderId);
}

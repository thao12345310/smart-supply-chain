...
The database schema and seed data are defined by versioned SQL migration files
under \texttt{db/migration}, named after the Flyway convention
(\texttt{V1}, \texttt{V2}, \dots). Migration-based schema management was
selected because database changes must be repeatable across development and
demonstration environments. Flyway documentation describes migrations as
versioned scripts that capture incremental database changes and run in a
consistent order \cite{flywayMigrations}; the project follows this convention,
with the scripts applied to PostgreSQL in version order when an environment is
prepared. This directly supports the requirement that the system be deployable
and maintainable without manually editing the database for each environment.

...
recovery. The credit-limit rule, which is presently implemented and unit-tested as
a domain method but not yet invoked during sales-order approval, should be wired
into the approval path so that orders exceeding a customer's available credit are
rejected automatically. The failed-delivery mechanism should add an explicit
database-level recovery marker so that repeated calls cannot restore the same
goods issue twice. The reporting module should be validated with larger sample
data.

...
procure-to-stock and order-to-cash processes. The backend contains 176 Java
classes with 11,821 lines of source code, while the frontend contains 10,575
lines of JavaScript/JSX source code, measured with the \texttt{cloc} tool
excluding blank and comment lines. The system implements the core modules
required by the project scope: master data, purchasing, goods receipt,
inventory, lot management, sales, goods issue, invoice, accounting, customer
payment, delivery, dashboard, authentication, and user management.
...
Several limitations remain. The project is still a prototype. Although it
includes an accounting module that records simplified double-entry journal
entries (each with a debit and a credit account) automatically from source
documents and tracks customer payments, it has not yet implemented full
financial reporting such as balance sheets and income statements, period-end
closing, or integration with external accounting software, and it does not yet
support automatic route optimization, barcode scanning, or demand forecasting. Some verification results should be completed with a
